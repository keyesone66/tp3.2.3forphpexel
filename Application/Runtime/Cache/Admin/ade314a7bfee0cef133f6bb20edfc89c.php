                      ...
                      <?php if(is_array($auths)): $i = 0; $__LIST__ = $auths;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$auth): $mod = ($i % 2 );++$i; if($auth["auth_pid"] == 0 ): ?><tr>
                                  <td>
                                      ...
                                      <input type="checkbox" class="chk" name="ids[]"
                                        <?php if(in_array(($auth["auth_id"]), is_array($role["role_auth_ids"])?$role["role_auth_ids"]:explode(',',$role["role_auth_ids"]))): ?>checked="checked"<?php endif; ?>
                                       value="<?php echo ($auth["auth_id"]); ?>">
                                      <?php echo ($auth["auth_name"]); ?>
                                  </td>
                                  <td>
                                    
                                    <?php if(is_array($auths)): $i = 0; $__LIST__ = $auths;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$at): $mod = ($i % 2 );++$i; if($at["auth_pid"] == $auth["auth_id"] ): ?><input type="checkbox" class="child" name="ids[]"
                                          <?php if(in_array(($at["auth_id"]), is_array($role["role_auth_ids"])?$role["role_auth_ids"]:explode(',',$role["role_auth_ids"]))): ?>checked="checked"<?php endif; ?>
                                         value="<?php echo ($at["auth_id"]); ?>">
